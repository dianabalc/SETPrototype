sap.ui.define([
	...
	return BaseObject.extend("com.hilti.set.model.GroupSortState", {
		 ...
		 * @alias com.hilti.set.model.GroupSortState
		 */
		...
		/**
		 * Groups by Number, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "Number") {
				...
				this._oViewModel.setProperty("/sortBy", "Number");

				aSorters.push(
					new Sorter("Number", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "Desc");
			}
